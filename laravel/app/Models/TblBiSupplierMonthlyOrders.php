...
class TblBiSupplierMonthlyOrders extends Model
{
    use HasFactory;
    ...
    {
        $sql = "INSERT INTO tbl_bi_supplier_monthly_orders (siire_code, bunrui_code, brand_code, `year_month`, order_amount,
                                             order_quantity, created_at, created_user_id, updated_at, updated_user_id)
                ...
                FROM (SELECT DISTINCT meisai.meisai_scm_code,
                                      ...
                                      syouhin.bunrui_code,
                                      syouhin.brand_code,
                                      DATE_FORMAT(tyumon.tyumon_date_value, '%Y%m')                                     AS `year_month`,
                                      meisai.hanbai_price                                                               AS order_amount,
                                      meisai.hanbai_number                                                              AS order_quantity
                      ...
                      WHERE tyumon.tyumon_date_value BETWEEN ? AND ?
    ...
    public static function aggregateMonthlyWebike($startDate, $endDate, $limit = 0, $offset = 0): int
    {
        $sql = "INSERT INTO tbl_bi_supplier_monthly_orders (siire_code, bunrui_code, brand_code, `year_month`, order_amount,
                ...
                SELECT 'all'                                         AS siire_code,
                       ...
                       0                                             AS brand_code,
                       DATE_FORMAT(tyumon.tyumon_date_value, '%Y%m') AS `year_month`,
                       SUM(meisai.hanbai_price)                      AS order_amount,
                       SUM(meisai.hanbai_number)                     AS order_quantity,
                       NOW()                                         AS created_at,
                       'admin'                                       AS created_user_id,
                       NOW()                                         AS updated_at,
                       'admin'                                       AS updated_user_id
                ...
                GROUP BY syouhin.bunrui_code, DATE_FORMAT(tyumon.tyumon_date_value, '%Y%m')";
        $bindParams = [$startDate, $endDate];
        if ($limit > 0) {
            $sql .= "
                    LIMIT ? OFFSET ? ";
    ...
    public static function aggregateOyaBunrui($oyaBunruiCol, $startMonth, $endMonth, $limit = 0, $offset = 0): int
    {
        ...
        $sql = "INSERT INTO tbl_bi_supplier_monthly_orders (siire_code, bunrui_code, brand_code, `year_month`, order_amount,
                ...
                FROM (SELECT a.siire_code,
                             ...
                             a.order_amount,
                             a.order_quantity
                      FROM tbl_bi_supplier_monthly_orders AS a
                      ...
                      GROUP BY a.siire_code, a.bunrui_code, b.$oyaBunruiCol, a.year_month, a.order_amount, a.order_quantity) AS c
